...
def run_branches_with_soft_boost(query: str, branches: List[Dict[str, Any]]) -> List[CandidateChunk]:
    """
    Run search with soft boosting instead of hard filtering.
    ...
    for branch in branches:
        ...
        # If not in cache, perform the search with soft boosting
        with ChromaClient() as client:
            ...
            try:
                ...
                semantic_results = collection.query(
                    ...
                    n_results=min(50, branch_limit * 3),  # Get 3x more than needed for boosting
                ...
                logger.error(f"Branch search with soft boosting failed: {e}", exc_info=True)
...
def run_branches(query: str, branches: List[Dict[str, Any]]) -> List[CandidateChunk]:
    """Original function - now calls the soft boosting version."""
    return run_branches_with_soft_boost(query, branches)
