...
def _get_sample_sentences_for_facet_value(client: ChromaClient, facet: str, value: str, limit: int = 5) -> List[str]:
...
def rebuild_all_facet_value_vectors(trace_id: Optional[str] = None) -> int:
    """Rebuild all facet-value vectors from the corpus."""
    with ChromaClient() as client:
        if not client._connected:
            logger.warning("Not connected to Chroma, cannot rebuild facet vectors")
            return 0
...
def rebuild_all_facet_vectors(trace_id: Optional[str] = None) -> int:
